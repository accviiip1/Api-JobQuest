    ...
    if (!data?.length) {
      ...
      const emailHTML = `
        ...
        <body style="color:#000;font-size:16px">
          ...
          <p>Nếu bạn không phải là người yêu cầu thay đổi mật khẩu hoặc có bất kỳ câu hỏi nào khác, vui lòng liên hệ với chúng tôi ngay lập tức qua địa chỉ email <a href="mailto:SDU-JobQuestofficial@gmail.com">SDU-JobQuestofficial@gmail.com</a> của chúng tôi.</p>
      ...
      var mailOptions = {
        ...
        to: `${data[0].email}`,
        subject: "SDU-JobQuest || Xác Nhận Tài Khoản và Tạo Mật Khẩu Mới",
        html: emailHTML,
      };
...
export const loginWithGoogle = async (req, res) => {
  ...
  if (!token) {
    return res.status(400).json("Token không được để trống!");
  }
  ...
  try {
    ...
    db.query(checkUserQuery, [email], async (err, data) => {
      ...
      if (data.length > 0) {
        // User đã tồn tại, cập nhật google_id nếu chưa có và tạo token
        const user = data[0];
        const secret = process.env.MY_SECRET;
        if (!secret) return res.status(500).json("Lỗi cấu hình máy chủ: thiếu MY_SECRET");
        ...
        if (!user.google_id) {
          ...
          db.query(updateQuery, [googleId, user.id], (updateErr) => {
            if (updateErr) {
        ...
        const userValues = [name, email, googleId];
        ...
        db.query(insertUserQuery, userValues, (err, result) => {
          ...
          const jwtToken = jwt.sign({ id: result.insertId }, secret, { expiresIn: "7d" });
    ...
    return res.status(401).json("Token Google không hợp lệ!");
  }
};
